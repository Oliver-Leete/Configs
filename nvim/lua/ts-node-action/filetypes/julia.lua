...
local toggle_julia_boolean = function(node) return tostring(helpers.node_text(node) ~= "true") end
...
local julia_trim_begin = function(node)
    ...
    for n, _ in node:iter_children() do
        ...
        if t ~= "begin" and t ~= "end" and t ~= "" then table.insert(ret, t) end
    end
...
local julia_func_long = function(node)
    ...
    if body[#body]:sub(1, 7) ~= "return " then body[#body] = "return " .. body[#body] end
...
local func_exp = function(node)
    ...
    for i, text in pairs(ret) do
        if i ~= 1 and i ~= #ret then ret[i] = text end
    end
...
local to_first_arg = function(node, do_n)
    ...
    if #body > 3 then
        body[1] = "begin"
        if body[#body - 1]:sub(1, 7) == "return " then body[#body - 1] = body[#body - 1]:sub(8, #body[#body - 1]) end
...
local toggle_do = function(node)
    for n, _ in node:iter_children() do
        if n:type() == "do_clause" then return to_first_arg(node, n) end
    end
...
local to_do_clause = function(node)
    ...
    if node:child(1):type() == "function_expression" then
        ...
        if func:child(0):type() == "parameter_list" then params = params:sub(2, #params - 1) end
        i = 0
        for c, _ in func:iter_children() do
            local text = helpers.node_text(c)
            if i > 0 and text ~= "->" then table.insert(ex, text) end
            i = i + 1
        end
    ...
    elseif node:child(1):type() == "function_definition" then
        ...
        for c, _ in func:iter_children() do
            local text = helpers.node_text(c)
            if i > 1 and text ~= "end" then table.insert(ex, text) end
            i = i + 1
        end
        do_str = do_str .. params
    end
