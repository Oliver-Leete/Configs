...
module XMonad.Actions.ProfilesLocal
  ( -- * Overview
    -- $overview
...
--------------------------------------------------------------------------------
-- | Prompt for adding a workspace id to a profile, changed to act on current profile only.
addWSToProfilePrompt :: XPConfig -> X()
addWSToProfilePrompt c = do ws <- gets (W.workspaces . windowset)
                            pws <- profileWorkspaces =<< currentProfile
                            p <- currentProfile
                            mkXPrompt (ProfilePrompt "Add WS: ") c (mkComplFunFromList' c (map W.tag ws \\ pws)) (`addWSToProfile` p)
...
removeWSFromProfilePrompt c = do pws <- profileWorkspaces =<< currentProfile
                                 p <- currentProfile
                                 mkXPrompt (ProfilePrompt "Remove WS: ") c (mkComplFunFromList' c pws) (`removeWSFromProfile` p)
...
profileLogger formatFocused formatUnfocused = do
  ...
  return $ Just $ foldl (\a b -> a ++ " " ++ b) "" $ reverse $ format p <$> hm
  where
...
--------------------------------------------------------------------------------
-- | Run a command on the nth workspace of the current profile.
withNthProfileWorkspace :: Int -> (WorkspaceId -> WindowSet -> WindowSet) -> X ()
withNthProfileWorkspace i action = do
      pw <- profileWorkspaces =<< currentProfile
      maybe (return ()) (windows . action) (pw !? i)
